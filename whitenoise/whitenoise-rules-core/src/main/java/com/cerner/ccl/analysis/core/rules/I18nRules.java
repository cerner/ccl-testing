...
/**
 * A {@link AnalysisRule} that identifies the following issues with internationalization:
 *   1. Instances where uar_i18nGetMessage accidentally has a duplicate i18n key value
 *
...
public class I18nRules extends TimedDelegate {
    ...
    protected Set<Violation> doMeasuredAnalysis() throws JDOMException {
        ...
        for (final Element uarGetMessage : uarGetMessages) {
        	...
        	//The i18n key has not been added to the collection yet, so there is no violation to add. So add the element to
        	//the map and finish
