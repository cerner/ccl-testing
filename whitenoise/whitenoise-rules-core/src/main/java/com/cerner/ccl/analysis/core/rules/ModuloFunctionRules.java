...
/**
 * A {@link AnalysisRule} that identifies the following issues with the modulo function:
 *   1. Modulo by 1 which is always 0
 *   2. Reversed modulo parameters
 *
    ...
    protected Set<Violation> doMeasuredAnalysis() throws JDOMException {
        ...
        for (final Element call : selectNodesByName("CALL.")) {
            ...
            if (call.getChildren().size() != 3)
                throw new JDOMException("Unhandled number of child elements to call mod() statement at line [" + getLineNumber(call) + "]");

            final Element param1 = call.getChildren().get(1);
            final Element param2 = call.getChildren().get(2);

            //The rule will assume that any hardcoded integer in the first parameter is an indication of reversed parameters
