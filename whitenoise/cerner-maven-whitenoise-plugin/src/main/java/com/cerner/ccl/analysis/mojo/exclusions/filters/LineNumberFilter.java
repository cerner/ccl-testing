...
/**
 * A {@link ViolationFilter} that will filter out a violation by line number. If the violation has no {@link Violation#getLineNumber() line number} associated with it, then this filter will not mark
 * it as excluded.
 * 
 * @author Joshua Hyde
 * 
...
public class LineNumberFilter implements ViolationFilter {
    ...
    public boolean exclude(final String scriptName, final Violation violation) {
        ...
        if (violation.getLineNumber() == null) {
            LOGGER.warn("An exclusion of line number {} was set, but violation {} has no line number information available.", Integer.valueOf(lineNumber), violation);
            return false;
        }
