...
/**
 * An {@link AnalysisRule} that flags any instance where the size() function is in the following incorrect ways
 * 1. size() on record list without the appropriate size option of 5
 * 2. size() on any string variable without nesting within a trim() function
 *
...
public class SizeFunctionRules extends TimedDelegate {
    ...
    protected Set<Violation> doMeasuredAnalysis() throws JDOMException {
        ...
        for (final Element sizeFunction : selectNodesByName("CALL.")) {
            ...
            if (!getCclName(sizeFunction).equalsIgnoreCase("SIZE")) continue;
            ...
            if (isElementARecordList(sizeFunction)) {
                if (!sizeOption.equalsIgnoreCase("5")) violations.add(new SizeOfRecordMemberViolation(sizeOption, getLineNumber(sizeFunction)));
            } else {
                if (sizeFunction.getChildren().size() < 3 || sizeOption.equalsIgnoreCase("1")) {
                    final Element firstParam = sizeFunction.getChildren().get(1);

                    if (!firstParam.getName().equalsIgnoreCase("CALL.") && !getCclName(firstParam).equalsIgnoreCase("TRIM") && firstParam.getName().equalsIgnoreCase("NAME")) {
    ...
    private String getSizeFunctionOptionNumber(final Element sizeFunction) {
        if (sizeFunction.getChildren().size() < 3) return "1";
    ...
    private boolean isElementARecordList(final Element sizeFunction) throws JDOMException {
        ...
        // If the first param is not MEMBER then it is impossible for this to be a list item. So, lets check for that and
        // return out of here
        ...
        // Does the last member name appear in the declaration of a record structure of a list item? The following XPath locates the following:
        // A record structure declaration whose record name matches the name of the first MEMBER./NAME element. This should be the name
        // of the record structure, and ensures that that record structure has a field element whose name matches the last MEMBER./NAME element
        // which is the item the size function is actually operating on, and lastly ensuring that that field element is defines as having an OCCUR
        // which should identify it as a list
        for (final Element record : allRecords) {
            if (selectNodes(
                    record,
 ".[COMMA./NAME/@text = '" + getCclName(firstParam) + "' and .//FIELD.[NAME/@text = '" + lastMemberName.getAttributeValue("text")
                    + "' and OCCUR.]]").size() > 0) {
                return true;
            }
        }
        ...
        // If we were not able to find a record structure declaration for the item then we must assume that the record was globally
        // defined, and we will make one last ditch effort to scan the source code for instances where the member is referenced as
        // a record list item. If we find such an occurrence then we should assume it's a list and therefore must be accompanied by a
        // second size parameter of 5
        for (final Element member : allMembers) {
            if (selectNodes(member,
 ".[NAME[1]/@text = '" + getCclName(firstParam) + "' and ARRAY.[NAME[1]/@text = '" + lastMemberName.getAttributeValue("text") + "']]").size() > 0) {
                return true;
            }
        }
