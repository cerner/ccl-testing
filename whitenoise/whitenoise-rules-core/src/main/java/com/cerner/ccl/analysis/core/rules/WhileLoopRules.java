...
public class WhileLoopRules extends TimedDelegate {
    ...
    protected Set<Violation> doMeasuredAnalysis() throws JDOMException {
        ...
        for (final Element whileLoop : whileLoops) {
            //The way I am going to do this check is pretty good but not perfect. Basically I will look at the conditional
        	//portion of the while loop and locate all NAME variables. At least one of those name variables will be by
        	//conditional, so I will then scan the body of the while loop looking for instances where the conditional is
        	//referenced on the left hand side of an operator to ensure that something in the conditional is getting a new
        	//value periodically.
