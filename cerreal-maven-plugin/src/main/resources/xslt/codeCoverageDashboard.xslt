...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        ...
        <xsl:element name="body">
            ...
            <div class="sectionDescription">This section displays the aggregate code coverage of all *.inc test programs for each source programs listed.
                A
                line of code is considered covered if the line was covered by at least one test program in the test suite</div>
            <br />
            <br />
            <xsl:call-template name="writeTestSuiteCoverageSection" />
            <hr />

            <xsl:for-each select="/coverageSummary">
                <xsl:choose>
                    <xsl:when test="skipTestSourceCoverage='true'">
                        <div>Skipping it.</div>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template name="writeTestSuiteCoverageSection">
        ...
        <h5>
            <i>*Code coverage percentage is calculated by dividing the covered lines by the sum of the covered and not covered lines. Not
                executable lines are not
                factored into code coverage calculations...</i>
        </h5>
    </xsl:template>
    ...
    <xsl:template name="writeCoverageTableRow">
        ...
        <tr>
            ...
            <td>
                <xsl:value-of select="$notCoveredLines" />
            </td>
            <td>
                <xsl:value-of select="$lines - $coveredLines - $notCoveredLines" />
            </td>
            <td>
                <xsl:choose>
